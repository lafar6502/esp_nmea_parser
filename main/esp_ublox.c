...
const int ublox_uart_num = UART1;
/*
...
uint8_t UBLOX_fill_buffer_UBX(uint8_t *buffer, uint8_t len)
{
	...
	//esp version - read len bytes from uart1
	int length;
	int length = uart_read_bytes(ublox_uart_num, buffer, len, 1000);
...
uint32_t UBLOX_construct_telemetry_UBX(uint8_t *buffer, uint32_t sequence)
{
	...
	float temperatureF1 = 0.0; // (float)AD15data * 0.30402 - 273.15 + TEMP_OFFSET;				// converting raw ADC reading to �C
